...
export default function CreateEventScreen() {
  ...
  const handlePickPhoto = async () => {
    ...
    if (!result.canceled && result.assets[0]) {
      ...
      try {
        const publicUrl = await uploadEventCover(localUri);
        if (publicUrl) {
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      setError(
        isNetworkFailure
          ? "Can't reach the server. Check your internet connection and that EXPO_PUBLIC_SUPABASE_URL in .env is a public URL (e.g. https://xxx.supabase.co), not localhost. Restart with: npx expo start -c"
          : raw
      );
      setCreating(false);
    }
